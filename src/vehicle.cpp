...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions) {
   ...
   */
   ...
   vector<Vehicle> trajectory, best_trajectory;
   vector<float> costs;
   float lowest_cost = 1.0e15;
   for (int s=0; s < possible_states.size(); s++){
        trajectory = generate_trajectory(possible_states[s], predictions);
        costs.push_back(calculate_cost(*this, predictions, trajectory));
        if (costs[s] < lowest_cost){
            lowest_cost = costs[s];
   ...
   */
  return best_trajectory; //generate_trajectory("KL", predictions);
}
